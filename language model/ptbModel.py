...
Glopath="output/"
TRAIN_DATA=Glopath+"ptb.train" #训练数据路径
EVAL_DATA=Glopath+"ptb.valid"  #验证数据路径
TEST_DATA=Glopath+"ptb.test"   #测试数据路径

HIDDEN_SIZE=300        #隐藏层规模
NUM_LAYERS=2           #深层循环神经网络LSTM结构的层数
VOCAB_SIZE=10000	   #词典规模
TRAIN_BATCH_SIZE=20		#训练数据batch的大小
TRAIN_NUM_STEP=35		#训练数据的截断距离

EVAL_BATCH_SIZE=1		#测试数据的batch大小
EVAL_NUM_STEP=1			#测试数据截断长度
NUM_EPOCH=5				#使用训练数据的轮数
LSTM_KEEP_PROB=0.9		#LSTM结点不被dropout的概率
...
class PTBModel(object):
	def __init__(self,is_training,batch_size,num_steps):
		...
		#定义每一步的输入和预期输出，两者维度都是[batch_size,num_steps].
		...
		#定义输出列表。在这里现将不同时刻的LSTM结构的输出手机起来，再一起提供给softmax层
		outputs=[]
		...
		#在softmax层：将RNN每个位置上的输出转化为各个单词的Logits
...
def run_epoch(session,model,batches,train_op,output_log,step):
	...
	#训练一个epoch
	for x,y in batches:
		#在当前batch上运行train_op并计算损失值，交叉熵损失函数计算的就是下一个恶单词给定单词的概率
